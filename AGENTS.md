...
- A pipeline orchestrator (`src/core/`) that runs all 16 steps and returns `PipelineExecutionResult`.
...
- Step 14 (Write Documentation) implemented in `src/docs/documentation-writer.ts` — writes Markdown from `ProjectKnowledge`.
